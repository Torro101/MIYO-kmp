...
class EdgeDetector(private val context: Context) {
        ...
        private suspend fun detectBounds(imageSource: ImageSource): Rect? {
                ...
                try {
                        ...
                        val pixels = try {
                                ...
                                // Guard against OOM on very tall webtoon strips: the pixel
                                // array is width*height*4 bytes. If the decoded bitmap is
                                // unreasonably large (e.g. a 12000px tall page at 1080px wide
                                // = ~52MB just for the IntArray), skip edge detection
                                // entirely and let the page render without trimming.
                                val pixelCount = width.toLong() * height.toLong()
                                if (pixelCount > MAX_PIXELS_FOR_EDGE_DETECTION) {
                                        return null
                                }
                                IntArray(pixelCount.toInt()).also {
        ...
        companion object {
                ...
                // 8M pixels = ~32MB IntArray. Pages larger than this skip edge
                // detection to avoid OOM on memory-constrained devices.
                private const val MAX_PIXELS_FOR_EDGE_DETECTION = 8_000_000L
        }
}
