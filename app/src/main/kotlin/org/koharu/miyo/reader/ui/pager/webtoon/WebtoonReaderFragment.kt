...
import org.koharu.miyo.reader.ui.pager.BaseReaderFragment
...
class WebtoonReaderFragment : BaseReaderFragment<FragmentReaderWebtoonBinding>(),
        ...
        private fun RecyclerView.findCurrentPagePosition(): Int {
                ...
                // Use a robust centerY: the bottom of the viewport minus the gap
                // between pages, but fall back to the viewport center when the gap
                // is 0 or when the computed Y is out of range (e.g. on the very
                // first/last page where there is no gap above/below). Without this
                // fallback, findCurrentPagePosition() returns NO_POSITION and the
                // reader cannot save/restore the current page, causing it to jump
                // back to page 0 after a config change or chapter switch.
                val gap = resources.getDimension(R.dimen.webtoon_pages_gap)
                val centerY = if (gap > 0f && height - gap > 0f) {
                        height - gap
                } else {
                        height / 2f
                }
